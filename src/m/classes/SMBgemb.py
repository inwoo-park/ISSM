...
class SMBgemb(object):
    ...
    def __init__(self, *args):  # {{{
        ...
        self.lon_mappedforcing = np.nan    #The longitude coordinate of each mapped forcing location (m). Of size number
                                           # of forcing points. Use if ismappedforcing and ismappingusingneighbors are true.
        self.x_mappedforcing = np.nan      #The x coordinate of each mapped forcing location (m), in same projection as the mesh. Of size number
                                           # of forcing points. Use if ismappedforcing and ismappingusingneighbors are true.
        self.y_mappedforcing = np.nan      #The y coordinate of each mapped forcing location (m), in same projection as the mesh. Of size number
                                           # of forcing points. Use if ismappedforcing and ismappingusingneighbors are true
    ...
    def __repr__(self):  # {{{
        ...
        s += '{}\n'.format(fielddisplay(self,'ismappingneighborxy','set to true if ismappingusingneighbors is true, and the forcing regular grid is x,y instead of lat,lon (default false)'))
        ...
        s += '{}\n'.format(fielddisplay(self,'x_mappedforcing','The x coordinate of each mapped forcing location (m in the same projection as the model mesh) for ismappedforcing and ismappingusingneighbors options, if ismappingneighborxy=true. Size number of forcing points.'))
        s += '{}\n'.format(fielddisplay(self,'y_mappedforcing','The y coordinate of each mapped forcing location (m in the same projection as the model mesh) for ismappedforcing and ismappingusingneighbors options, if ismappingneighborxy=true. Size number of forcing points.'))
        ...
        if self.ismappedforcing:
            ...
            if self.ismappingusingneighbors:
                ...
                if self.ismappingneighborxy:
                    md = checkfield(md,'fieldname','smb.x_mappedforcing','size',[sizeta[0]-1],'NaN',1,'Inf',1)
                    md = checkfield(md,'fieldname','smb.y_mappedforcing','size',[sizeta[0]-1],'NaN',1,'Inf',1)
                else:
